...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    !    SwitchMatTop(:,:)     = (PreyMat(:,:) * gkernel)**switch_feeding(:)           ! Passive (ns=1) or active (ns=2) switching term [jpred,jprey], ns replaced by switch_feeding - Grigoratou Dec19
       ...
       if (graz(jpred).gt.0.0) then ! if non-zero max grazing rate
          ...
          do jprey=1,npmax
             if (biomass(iCarb,jprey).gt.0.0.and.food2.gt.0.0) then ! if any prey food available
                ...
                zoolimit(jpred,jprey) = tmp1 *(gkernel(jpred,jprey)*palatability(jprey)*biomass(iCarb,jprey))**switch_feeding(jpred)/food2 ! food limitation calulation for zooplankton - Grigoratou May 2019					 				 
