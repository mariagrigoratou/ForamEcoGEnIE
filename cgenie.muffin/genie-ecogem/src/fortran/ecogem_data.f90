  ...
  SUBROUTINE sub_init_plankton()
    ...
    do jp=1,npmax
	 ...
	 carnivory (jp)      = 0.0
          ...
          pp_optC(jp)         = 10.0              !optimum PP ratio
          pp_sig_C(jp)        = 2.0               !standard deviation for PP ratio
          mort_protect(jp)    = trophic_tradeoff *0.7 
    ...
    do jp=1,npmax ! grazing kernel (npred,nprey)
       ...
       ppopt_mat(:,jp)=pp_optC * volume(:) ** pp_opt_b   !added an optimal predator-prey length ratio for each plankton group, Grigoratou, Dec18
       ppsig_mat(:,jp)=pp_sig_C * volume(:) ** pp_sig_b  !added an optimal standar deviation for predator-prey length ratio for each plankton group, Grigoratou, Dec18
    enddo
    !PRINT*,ppsig_mat  
